...
library(gam)
## Fit a GAM
 gam1 <- gam(Outcome~s(Age)+s(Experience)+s(Income)+s(Family)+s(CCAvg)+Education+s(Mortgage)+CD_Account+Online+CreditCard,
               family='binomial', data=train_data)
 plot(gam1, col='blue') 
```
...
## Model evaluation on Test data
 ...
 lift_chart <- lift(test_data$Outcome~lm_full_pred+lm_bwd_pred+gam1_pred, class = 'Yes')
...
# generate a data frame with categorical predictors being represented as dummy variables
 x_train_nn <- model.matrix(~.-1, data = train_data)
...
# generate and standardize the data frame for the test data as well
 x_test_nn <- model.matrix(~.-1, data = test_data)
...
library(neuralnet)
set.seed(7)
 nn1 <- neuralnet(Outcome =='Yes'~.,data = x_train_nn, hidden = c(4), linear.output = F)
```
